...
if (nargin==4) && isempty(value) %prompt user to select slices for a 
                                 %user given width
    value=SelectValue(coordinate,object,width); 
end

if (nargin==3) && isempty(width) %prompt user to select slices for a 
                                 %user given value without width
    width=0.0;
end

if (nargin<3) || isempty(value) % prompt user to select slices
    %set width equal to zero
    width = 0.0;
    value=SelectValue(coordinate,object,width);       
end
