...
elseif isstruct(data)
    ...
    if isfield(data,'FileOption')
        if isnumeric(data.FileOption)
            option=sprintf('%d',data.FileOption);
        else
            option=sprintf('''%s''',data.FileOption);
        end
        data.Summary=sprintf('%s (%s), %s',[name ext],option,format);
    else
