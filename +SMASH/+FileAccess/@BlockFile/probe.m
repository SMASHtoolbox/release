% PROBE Probe the column block file
%
% Syntax:
...
% open file and look for purely numerical rows
fid=fopen(object.FullName,'r');     
finish=onCleanup(@() fclose(fid));

report=[];
while ~feof(fid) % file scan
    report(end+1).Start=ftell(fid);
    skip=0;
    while true % block scan
        numcol=0;
        format='';
        temp=fgets(fid);
        in=strtrim(temp);
        ...
        if isempty(format) || (numcol<mincol)
            ...
            fscanf(fid,format,[numcol inf]);
            break
        end
    end
    report(end).HeaderLines=skip;
    report(end).NumberColumns=numcol;
    report(end).Format=format;
end
