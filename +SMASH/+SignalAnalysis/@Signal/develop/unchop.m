...
% values are determined from local averages over a specified duration.  
%   [range,uncertainty]=unchop(object,duration);
% Signal range is 

%local averaging are returned in the output
% structure "report", along with estimated uncertainties
...
%
function [range,uncertainty]=unchop(object,duration)
...
% look for multiple chops
temp=fft(object,'RemoveDC',true);
[~,index]=max(temp.Data);
f0=temp.Grid(index);
