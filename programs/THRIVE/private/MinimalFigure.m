...
message{end+1}=[indent 'Click on lines to display (x,y) data coordinates.'];
message{end+1}=[indent 'Press right mouse button (control-click) for additional options'];
message{end+1}=' ';
message{end+1}='9. ROI statistics ';
...
%     visible=get(hc(ii),'Visible');
%     if any(kk(:)) && strcmpi(visible,'on') % create report figure
% 
% 
%         label={};
%         label{end+1}=sprintf(' Horizontal range: %+g to %+g ',xmin,xmax);
...
%         switch get(hc(ii),'Type')
%             case 'line'
%              set(fig,'Name','ROI line statistics');
%              label{end+1}='';
%              label{end+1}=sprintf(' Horizontal mean: %+g ',mean(x(kk)));
%              label{end+1}=sprintf(' Horizontal deviation: %+g ',std(x(kk)));  
%              label{end+1}='';
%              label{end+1}=sprintf(' Vertical mean: %+g ',mean(y(kk)));  
%              label{end+1}=sprintf(' Vertical deviation: %+g ',std(y(kk)));
%              %a=polyfit(x(kk),y(kk),1);
%              %label{end+1}=sprintf('Slope: %+g',a(1));             
%              hl=line('Parent',ha,'XData',x(kk),'YData',y(kk));
%              axis(ha,'tight');
%              linkprop([hc(ii) hl],...
...
% % create dialog box
% gset=GUIsettings;
% Lx=2*gset.hmargin+2*gset.labelwidth+gset.editwidth;
% Ly=gset.buttonheight+(Ndata+1)*gset.textheight+4*gset.vmargin;
% dlg=findobj('Tag','MinimalFigure:ROIstatistics');
% if ishandle(dlg)
%     pos=get(dlg,'Position');
%     pos(3:4)=[Lx Ly];
%     popup=findobj(dlg,'Tag','ROIvariable');
%     popupvalue=get(popup,'Value');
%     delete(dlg);
% else
%     units=get(mainfig,'Units');
%     set(mainfig,'Units','characters');
%     figpos=get(mainfig,'Position');
%     set(mainfig,'Units',units);
%     pos=[figpos(1:2) Lx Ly];
%     popupvalue=3;
% end
% dlg=figure(...
%     'NumberTitle','off','Name','ROI statistics',...
%     'IntegerHandle','off','HandleVisibility','callback',...
%     'Tag','MinimalFigure:ROIstatistics',...
%     'Toolbar','none','Menubar','none','DockControls','off',...
%     'Units','characters','Position',pos,'Resize','off',...
%     'UserData',hrect);
% % create done button
% pos(2)=gset.vmargin;
% pos(3)=gset.buttonwidth;
% pos(4)=gset.buttonheight;
% pos(1)=(Lx-pos(3))/2;
% callback='h=get(gcbf,''UserData'');delete(gcbf);delete(h)';
% uicontrol('Style','pushbutton','Tag','Done',...
%     'units','characters','Position',pos,...
%     'String','Done','Callback',callback);
% % create output field(s)
% pos(1)=gset.hmargin+gset.labelwidth;
% pos(2)=pos(2)+pos(4)+gset.hmargin;
% pos(3)=gset.editwidth;
% pos(4)=Ndata*gset.textheight;
% editbox=uicontrol('Style','text',...
%     'Max',2,'Min',0,...
%     'Units','characters','Position',pos,...
%     'BackgroundColor',gset.textbgcolor);
% pos(1)=gset.hmargin;
% pos(3)=gset.labelwidth;
% pos(4)=gset.textheight;
% for ii=1:Ndata
%     axes('units','characters','Position',pos);
%     line([0 1],[0.5 0.5],'Color',get(hc(ii),'Color'));
%     axis off
%     pos(2)=pos(2)+pos(4);
% end
% % create variable popup
% pos(1)=gset.hmargin+gset.labelwidth;
% pos(2)=Ly-gset.vmargin-gset.textheight;
% pos(3)=gset.editwidth+gset.labelwidth;
% pos(4)=gset.textheight;
% popup=uicontrol('Style','popup','Tag','ROIvariable',...
%     'Units','characters','Position',pos,...
%     'String',{'mean(x)','std(x)','mean(y)','std(y)'},...
%     'Value',popupvalue,'Max',1,'Min',0,...
%     'BackgroundColor',gset.textbgcolor,...
%     'UserData',{editbox stats},...
%     'Callback',@ROIvariableUpdate);
% 
% % appearance formatting
% h=findobj(dlg,'Type','uicontrol');
% set(h,'FontName',GUIsettings('fontname'));
% %set(h,'Value',3);
% ROIvariableUpdate(popup);
% 
% function ROIvariableUpdate(src,varargin)
% 
...
% N=size(stats,1);
% for ii=1:N
%     label{ii}=sprintf('%.6g',stats(N-ii+1,value));
% end
% set(editbox,'String',label);
...
data.color='k';
data.linewidth=0.5;
hl=line('Parent',target,'UserData',data);
...
uimenu(hm,'Label','Remove overlay','Tag','delete',...
    'Callback',{@update_overlay,hl});
uimenu(hm,'Label','Set line color','Tag','color',...
    'Callback',{@update_overlay,hl},'Separator','on');
uimenu(hm,'Label','Set line width','Tag','width',...
    'Callback',{@update_overlay,hl});
update_overlay([],[],hl);
...
if ishandle(src)
    ...
    switch lower(tag)
        ...
        case 'shift'
            ...
            label{1}='Horizontal shift (additive)';
            label{2}='Vertical shift (additive)';
