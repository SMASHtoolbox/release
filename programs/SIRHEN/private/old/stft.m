...
function [output,tout,frequency,options]=stft(signal,time,options,localfunc)
...
%reverse_findex=numel(frequency):-1:2;
...
while index(end)<=numel(signal)
    ...
    %intensity=intensity.*conj(intensity);
    intensity=real(intensity.*conj(intensity)); % ensure real result
    if isempty(localfunc)
        output(:,k)=intensity;
    else
        %x=[-frequency(reverse_findex); frequency];
        %y=[intensity(reverse_findex); intensity];
        %result=feval(localfunc,x,y);
        result=feval(localfunc,frequency,intensity);
