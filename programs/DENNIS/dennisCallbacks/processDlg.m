...
h_circle = addblock(db, 'check', ' Use Circle ROI?'); 
...
% fix mac missing button problem (caused by inability to properly scale for
% macs - fixable by forcing resize)
...
set(h_back, 'Callback', {@processBack, mainFigure, h_circle});
set(h_backReset, 'Callback', {@processBackReset, mainFigure});
set(h_mask, 'Callback', {@processMask, mainFigure, h_circle});
set(h_maskReset, 'Callback', {@processMaskReset, mainFigure});
set(h_reverseMask, 'Callback', {@processReverseMask, mainFigure, h_circle});
...
set(h_circle, 'Callback', {@processCircle});
...
function processBack(src, event, mainFigure, h_circle)
obj = get(mainFigure, 'UserData');
ax = get(ancestor(src, 'figure'), 'CurrentAxes');
if get(h_circle, 'Value')
    ind = drawCircle(obj.detector.image, ax);
...
function processMask(src, event, mainFigure, h_circle)
obj = get(mainFigure, 'UserData');
ax = get(ancestor(src, 'figure'), 'CurrentAxes');
if get(h_circle, 'Value')
    ind = drawCircle(obj.detector.image, ax);
...
function processReverseMask(src, event, mainFigure, h_circle)
obj = get(mainFigure, 'UserData');
ax = get(ancestor(src, 'figure'), 'CurrentAxes');
if get(h_circle, 'Value')
    ind = drawCircle(obj.detector.image, ax);
...
ax = get(ancestor(src, 'figure'), 'CurrentAxes');
if get(h_circle, 'Value')
    [~, diam] = drawCircle(obj.detector.image, ax);
...
function processCircle(src, event)
v = version('-release');
yr = str2double(v(1:4));
okFlag = false;
if yr > 2018 || (yr == 2018 && strcmp(v(end), 'b'))
